...
# archivo de starter
#-------------------
#p1=SS_readdat_3.30("data.dat", verbose = TRUE, echoall = FALSE,section = NULL)
#p1
...
#---------------
  ...
  #### files of the "truth"
  #########################################
  bootcomm <- paste0(mcomm ," -nohess -nox")
  ...
  foreach(j = 1:Nboot) %dopar% {
    ...
    if(os!="windows")
      system(mcomm, show.output.on.console = FALSE)
  
    if(os=="windows")
      shell(mcomm, show.output.on.console = FALSE)
